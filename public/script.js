...
Pro reálnou editaci byste museli implementovat formulář a logiku pro aktualizaci dat v activeOrders.`);
    ...
    // --- Event Listenery ---
    // Přihlašovací formulář je nyní v index.html, takže zde není potřeba
    ...
    if (nastaveniRoleSelect) {
        nastaveniRoleSelect.addEventListener('change', (e) => {
            ...
            // Po změně role je nejlepší přesměrovat na přihlašovací stránku
            // aby se načetl správný dashboard s novými oprávněními
            setTimeout(() => {
                logout();
            }, 2000); // Dáme uživateli čas přečíst zprávu
        });
    }
